    ...
    return (
        <aside className="flex h-screen w-36 shrink-0 flex-col overflow-hidden bg-[#0b0f19] text-zinc-300">
            <div className="shrink-0">
                <Link href="/admin/submissions" className="block bg-zinc-100 py-4 text-center text-lg font-bold tracking-wide text-zinc-900">
                    3HATS
                </Link>
            </div>
            <nav className="flex min-h-0 flex-1 flex-col items-center gap-9 overflow-y-auto py-10">
                {navItems.map(({ label, icon: Icon, href }) => (
                    <Link
                        href={href}
                        key={label}
                        onClick={() => console.log(`Navigate: ${label}`)}
