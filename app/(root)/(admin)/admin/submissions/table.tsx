    ...
    return (
        <>
            ...
            <div className="flex items-center justify-between border-t border-zinc-200 pt-3">
                <button type="button" onClick={onClear} className="text-sm text-zinc-500">Clear all</button>
                <button
                    type="button"
...
export default function Table({ artworks, total, isNext, page, pageSize, filterOptions }: TableProps) {
    ...
    const currentStatuses = (searchParams.get("status")?.split(",").filter(Boolean) ?? []) as ArtworkStatus[]
    ...
    const allSelected = artworks.length > 0 && artworks.every((item) => selectedIds.includes(item._id))
    ...
    const activeStatusTab: ArtworkStatus | "all" = currentStatuses.length === 1 ? currentStatuses[0] : "all"
    ...
    const activeFilterCount =
        currentStatuses.length + currentRegions.length + currentMediums.length + (currentDateFrom ? 1 : 0) + (currentDateTo ? 1 : 0)
            ...
            <div className="flex items-center gap-6 overflow-x-auto border-b border-zinc-200 px-6">
                {statusTabs.map((tab) => (
                    <button
                        ...
                        className={`whitespace-nowrap border-b-2 py-4 text-sm transition-colors ${activeStatusTab === tab.key
                        ...
                        {pageNumbers.map((pageNumber, index) =>
                            pageNumber === "ellipsis" ? (
                                ...
                                <PaginationItem key={pageNumber}>
                                    <PaginationLink href={buildUrl({ page: String(pageNumber) })} isActive={pageNumber === page}>
                                        {pageNumber}
                                    </PaginationLink>
                                </PaginationItem>
                            ),
