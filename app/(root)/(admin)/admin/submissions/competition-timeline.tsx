...
function EditableDateRange({ startId, endId, startValue, endValue, editable, onSaved }: EditableDateRangeProps) {
    const [isEditing, setIsEditing] = useState(false);
...
function TimelineRow({ icon, label, children, stacked }: TimelineRowProps) {
    const labelEl = (
        <p className="flex items-center gap-2.5 text-sm text-zinc-700 shrink-0">
            <span className="flex items-center justify-center size-6 text-zinc-500 shrink-0">
                {icon}
            </span>
            {label}
        </p>
...
export default function CompetitionTimeline({ dates }: CompetitionTimelineProps) {
    const [values, setValues] = useState(() => new Map(dates.map((item) => [item._id, item.date as string | Date])));
                ...
                <ul className="flex flex-col">
                    {entriesOpen && (
                        <TimelineRow icon={<Clock size={12} />} label="Entries open">
                            <EditableDate id={entriesOpen._id} value={values.get(entriesOpen._id) ?? entriesOpen.date} editable={isEditMode} onSaved={handleSaved} />
                        </TimelineRow>
                    )}
                    {entriesClose && (
                        <TimelineRow icon={<Clock size={12} />} label="Entries close">
                            <EditableDate id={entriesClose._id} value={values.get(entriesClose._id) ?? entriesClose.date} editable={isEditMode} onSaved={handleSaved} />
                        </TimelineRow>
                    )}
                    ...
                    {winnersAnnounced && (
                        <TimelineRow icon={<Clock size={12} />} label="Winners announced">
                            <EditableDate id={winnersAnnounced._id} value={values.get(winnersAnnounced._id) ?? winnersAnnounced.date} editable={isEditMode} onSaved={handleSaved} />
                        </TimelineRow>
                    )}
                </ul>
            </div>
